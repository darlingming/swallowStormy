...
public final class SerialVagueFactroy implements Serializable {
    ...
    private ApplicationVagueContainer avc;

    public SerialVagueFactroy(String serialPath) throws IOException {
        this.deSerial(serialPath);
    }

    public SerialVagueFactroy(InputStream is) throws IOException {
        this.deSerial(is);
    }
     ...
     * @throws IOException
     */
    private ApplicationVagueContainer deSerial(String serialPath) throws IOException {
     ...
     * @throws IOException
     */
    private ApplicationVagueContainer deSerial(InputStream is) throws IOException {
        Kryo kryo = ApplicationVagueContainer.getKryo();
        ...
        Map<T_extract_rule, Pattern> extractDoMainPatternRuleMap = applicationVagueContainer.getExtractDoMainPatternRuleMap();
        ...
        List<T_extract_rule> extractRuleList = applicationVagueContainer.getExtractRuleList();
    ...
    public T_ip_rule executeIP(final String ip, final String port) {
        ...
        if (!ip.isEmpty() && !port.isEmpty())
            this.ipResult = this.avc.getiPMap().get(ip + port);
    ...
    public boolean execute(final String[] fullDomain) {
        ...
        Map<String, List<Object>> domainEqualsMap = this.avc.getDomainEqualsMap();
        ...
        // domain ģ��ƥ��
        Set<Object> paramSet = avc.getAfDomain().serachResult(domain);
        if (!paramSet.isEmpty()) {
        ...
        // extract pattern
        Map<T_extract_rule, Pattern> extractPattern = avc.getExtractPatternRuleMap();
        ...
        Map<T_extract_rule, Pattern> extractDoMainPattern = avc.getExtractDoMainPatternRuleMap();
